...
void potentiometer::setup(int main_adress, float main_range_lower, int alternate_adress, float alternate_range_lower, int16_t *current_sysex_parameters_pointer, int alternate_initial_value, const std::function<void(int, int)> & apply_audio_parameter, int alternate_storage_adress) {
    ...
    this->main_range_upper = (main_adress == 14 && current_sysex_parameters_pointer) ? current_sysex_parameters_pointer[200] / 100.0 : 100.0; // mod_pot: 200
    this->alternate_adress = alternate_adress;
    this->alternate_range_lower = alternate_range_lower;
    this->alternate_range_upper = (current_sysex_parameters_pointer) ? current_sysex_parameters_pointer[alternate_adress == 10 ? 201 : (alternate_adress == 12 ? 202 : (alternate_adress == 16 ? 203 : alternate_adress))] / 100.0 : 100.0; // chord: 201, harp: 202, mod: 203
...
void potentiometer::force_update() {
    ...
    if (found && is_integer) {
        ...
        if (range_fraction >= 1.0f) {
            ...
            scaled_range = max(1, scaled_range);
        ...
        // Float parameter: map to [alternate_range, alternate_range_upper]
...
bool potentiometer::update_parameter(bool alternate_flag) {
    ...
    if (abs(potentiometer_old_value - potentiometer_smoothed_value) > threshold || trigger_change) {
        ...
        if (found && is_integer) {
            ...
            // Float parameter: map to [range, range_upper]
